...
namespace cs237 {
	...
	inline mat2x2<T> & mat2x2<T>::operator*= (mat2x2 const &mm)
	{
	    vec2<T> r0 = vec2<T>(this->mm.cv[0][0], this->mm.cv[1][0]);
	    this->cv[0][0] = dot(r0, mm.cv[0]);
	    this->cv[1][0] = dot(r0, mm.cv[1]);

	    vec2<T> r1 = vec2<T>(this->mm.cv[0][1], this->mm.cv[1][1]);
	...
	inline mat2x2<T> mat2x2<T>::inverse() const
	{
	    ...
	    return mat2x2<T>(
		s*this->cv[0][0], s*this->cv[1][0],
		s*this->cv[0][1], s*this->cv[1][1]);
	}
	...
	{
	    return mat2x2<T>(m1.cv[0] + m2.cv[0], m1.cv[1] + m2.cv[0]);
	}
