...
namespace OBJ {
  ...
  // scan one or more floats from a string
	...
	while (std::getline(inS, ln).good()) {
		...
		std::string firstTok = ln.substr(0, pos);
		std::string rest = ln.substr(ln.find_first_not_of(" \t", pos));
		if (firstTok.compare("newmtl") == 0) { // newmtl name
		  ...
		  // initialize mtl to default values
			...
			if ((0 <= ival) && (ival < 3)) {
			    ...
			    std::cerr << "Error: excepted \"illum [0-2]\"" << std::endl;
			}
		    }
		    else {
...
#ifdef MORE_OBJ_FEATURES
		else if ((firstTok.compare("map_Bump") == 0) || (firstTok.compare("map_bump") == 0)) {
		    /* ignore bump/height map */
		}
